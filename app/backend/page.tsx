...
export default function BackendPage() {
  ...
  async function searchReceipts(query?: string) {
    ...
    const { data, error } = await supabase.from("receipts").select("*, patients(name), receptionist(name)");
    console.log("searchReceipts", { searchQuery, data, error });
    if (data) {
    ...
    >
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-5">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {selectedReceipt && (
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-semibold text-slate-700">Receipt Items</p>
              <p className="mt-1 text-xs text-slate-600">Select items to refund</p>

              <div className="mt-4 space-y-2">
                {receiptItems.map((item) => (
                  <label
                    key={item.id}
                    className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 hover:bg-slate-50"
                  >
                    <input
                      type="checkbox"
              ...
              <div className="mt-4">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm outline-none focus:border-teal-400"
                />
              </div>
              ...
              <button
                ...
                className="mt-4 w-full rounded-2xl bg-red-600 px-5 py-2 text-sm font-semibold text-white hover:bg-red-500 disabled:opacity-50"
              >
          ...
          {lastRefund && (
            <div className="mt-6 rounded-2xl border border-green-200 bg-green-50 p-4">
              ...
              <button
                onClick={printRefund}
                className="mt-3 w-full rounded-2xl bg-green-600 px-5 py-2 text-sm font-semibold text-white hover:bg-green-500"
              >
