alter table public.patient_treatment_visits
  drop constraint if exists patient_treatment_visits_batch_row_sequence_uniq;

alter table public.patient_treatment_visits
  add constraint patient_treatment_visits_batch_row_sequence_uniq
  unique (clinic_id, import_batch_id, source_row_number, visit_sequence);
...
begin
  ...
  where v.import_batch_id = p_import_batch_id
    and (
      ...
      or (nullif(v.visit_date, '') is not null and public.safe_parse_iso_date(v.visit_date) is null)
  ...
  select
    ...
    v.import_batch_id,
    v.source_row_number,
    md5(c.id::text || ':' || v.import_batch_id || ':' || v.source_row_number || ':' || v.visit_sequence)
  from public.clinic_treatment_visit_import_staging v
  join public.clinics c
    on upper(c.code) = upper(v.clinic_code)
  ...
  on conflict (clinic_id, import_batch_id, source_row_number, visit_sequence) do nothing;
